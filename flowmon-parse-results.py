...
class Flow(object):
    __slots__ = ['flowId', 'delayMean', 'packetLossRatio', 'rxBitrate', 'txBitrate',
                 ...
                 'fct', 'pfrac_fct', 'txBytes', 'txPackets', 'rxPackets', 'rxBytes', 'lostPackets']
    def __init__(self, flow_el):
        ...
        #pfrac_fct = float(long(flow_el.get('timePfracRxPacket')[:-4]) - long(flow_el.get('timeFirstTxPacket')[:-4]))*1e-9
...
def main(argv):
    ...
    for sim in sim_list:
        for flow in sim.flows:
            #if flow.fct < 0.15 and flow.fct > 0.005:
                #print flow.flowId, flow.fct, flow.pfrac_fct
            if flow.fct == None:
                continue
            if flow.txBytes <= 80 * flow.txPackets + 4:
                continue
            flow_count += 1
            total_fct += flow.fct
            total_pfrac_fct += flow.pfrac_fct
            if tail_fct < flow.fct:
                tail_fct = flow.fct
            if pfrac_fct < flow.pfrac_fct:
                pfrac_fct = flow.pfrac_fct
	    ...
	    total_lost_packets += flow.lostPackets
            ...
            print "\tFCT: %.6f" % (flow.fct)
            """

    print (sorted(flow_list))
            
    print "Total flows %i" % flow_count
    print "Avg FCT: %.6f" % (total_fct * 1000 / flow_count) # ms
    print "Tail FCT: %.6f" % (tail_fct * 1000)
    print "Tail pfrac FCT: %.6f" % (pfrac_fct * 1000)
    if large_flow_count == 0:
        print "No large flows"
    else:
        print "Large Flow Avg FCT: %.4f" % (large_flow_total_fct * 1000 / large_flow_count)

    if small_flow_count == 0:
        print "No small flows"
    else:
	print "Small Flow Avg FCT: %.4f" % (small_flow_total_fct * 1000 / small_flow_count)
    ...
    print "Lost Flow Ratio: %.4f" % (loss_flow_count * 100 / flow_count)
