...
int LEAF_COUNT = 2;
int SERVER_COUNT = 12;
int PS_COUNT = 2;     // we chose the first 4 servers in one rack as the PS server.
int WORKER_COUNT = 10; //
int OVERSUB_RATIO = 2;
...
/* Compute important values based on the constants */
int Hash_Layer(int layerIndex) //hash one layer to a PS server
{
  int P1 = 953, P2 = 311;
  int P3 = 811, P4 = 127;
  return ((P1*layerIndex+P2)%LEAF_COUNT)*SERVER_COUNT + ((P3*layerIndex + P4)%PS_COUNT);
}
...
void InstallAllApplications (NodeContainer servers)
{
  ...
  flowLayerFilename << "tracerst/ps_flow_" << "mptcp" << "_" << MODELS_NAME[Model_chose] << "_bw" << BW_GBPS << "_lossrt" << LOSELESS_RATIO 
                      << "_" << SPINE_COUNT << "to" << LEAF_COUNT << "_s" << SERVER_COUNT << "_ps" << PS_COUNT << "_worker" << WORKER_COUNT
  ...
  {
    if (senderIndex % SERVER_COUNT < PS_COUNT || senderIndex % SERVER_COUNT >= PS_COUNT+WORKER_COUNT) continue;
    for (int layerIndex = 0; layerIndex < LAYER_COUNT; ++layerIndex)
    {
      if(LAYER_SIZE[layerIndex] < TENSOR_THRESHOLD)
      {
        fout << "flowId:" << flow_count << " " << "srcIP:" << senderIndex << " dstIP:" << Hash_Layer(layerIndex) << std::endl;
        flow_count++;
        InstallTcpApplication(servers, LAYER_SIZE[layerIndex],
                            senderIndex, Hash_Layer(layerIndex),
                             flow_count, layerIndex+1);
      }
      else
      {
        for (int rackIndex = 0; rackIndex < LEAF_COUNT ; ++rackIndex)
          for (int psIndex = 0; psIndex < PS_COUNT; ++psIndex)
          {
            int PS_tot_num = LEAF_COUNT * PS_COUNT;
            int PS_id = rackIndex * SERVER_COUNT + psIndex;
            fout << "flowId:" << flow_count << " " << "srcIP:" << senderIndex << " dstIP:" <<  PS_id << std::endl;
            flow_count++;
            InstallTcpApplication(servers, LAYER_SIZE[layerIndex] / PS_tot_num ,
                            senderIndex, PS_id,
                             flow_count, layerIndex+1);
          }
      }
...
void OutputMonitor (Ptr<FlowMonitor> flowMonitor)
{
  ...
  flowMonitorFilename << "tracerst/ps_flow_" << "mptcp" << "_" << MODELS_NAME[Model_chose] << "_bw" << BW_GBPS << "_lossrt" << LOSELESS_RATIO 
                      << "_" << SPINE_COUNT << "to" << LEAF_COUNT << "_s" << SERVER_COUNT << "_ps" << PS_COUNT << "_worker" << WORKER_COUNT
...
void SetCommandLine(CommandLine& cmd)
{
  ...
  cmd.AddValue("psNum", "Number of ps servers in one rack", PS_COUNT);
  cmd.AddValue("workerNum", "Number of ps workers in one rack", WORKER_COUNT);
